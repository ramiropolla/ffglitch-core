...
#define FFGLITCH_VERSION 0.1.010
...
static int transplicate(CAS9OutputContext *c9)
{
    ...
    while ( 42 )
    {
        ...
        dctx = dctxs[ipkt->stream_index];
        if ( dctx == NULL )
            continue;

        if ( jstreams0 != NULL && apply_fname != NULL )
        {
            size_t idx = frames_idx[ipkt->stream_index];
            json_object *jframes = jstframes[ipkt->stream_index];
            ...
            frames_idx[ipkt->stream_index]++;
        }

        if ( avcodec_send_packet(dctx, ipkt) < 0 )
        {
        ...
        if ( c9 != NULL )
        {
            if ( dctx->cas9_out == NULL )
            {
                av_log(c9, AV_LOG_ERROR, "failed to get output data\n");
                goto the_end;
            }
...
int main(int argc, char *argv[])
{
    ...
    if ( export_fname != NULL )
    {
        ...
        jversion = json_object_new_string(AV_STRINGIFY(FFGLITCH_VERSION));
