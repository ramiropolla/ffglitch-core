...
int ff_mjpeg_decode_dht(MJpegDecodeContext *s)
{
    ...
    if ( (s->avctx->cas9_export & (1 << CAS9_FEAT_DHT)) != 0 )
    {
        ...
        //  ]
        // }
        s->cas9_dht = json_object_new_object();
        jtables = json_object_new_array();
        json_object_object_add(s->cas9_dht, "tables", jtables);
    }
    else if ( (s->avctx->cas9_import & (1 << CAS9_FEAT_DHT)) != 0 )
    {
        json_object_object_get_ex(s->cas9_dht, "tables", &jtables);
...
cas9_dct_scan_new(MJpegDecodeContext *s)
{
    ...
    for ( size_t component = 0; component < s->nb_components; component++ )
    {
        ...
        for ( size_t line = 0; line < height; line++ )
        {
            ...
            if ( (s->avctx->cas9_export & (1 << CAS9_FEAT_Q_DC)) != 0 )
            {
                json_object_set_serializer(jcolumns,
                          ...
                          AVPacket *avpkt)
{
    ...
    if ( (avctx->cas9_import & (1 << CAS9_FEAT_DHT)) != 0 )
        s->cas9_dht = avpkt->cas9_sd[CAS9_FEAT_DHT];
...
eoi_parser:
            ...
            *got_frame = 1;
            ...
            if ( (s->avctx->cas9_export & (1 << CAS9_FEAT_DHT)) != 0 )
            {
                frame->cas9_sd[CAS9_FEAT_DHT] = s->cas9_dht;
                s->cas9_dht = NULL;
            }
