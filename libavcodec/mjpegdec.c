...
int ff_mjpeg_decode_dqt(MJpegDecodeContext *s)
{
    ...
    {
        AVFrame *f = s->picture_ptr;
        json_object *jframe = f->ffedit_sd[FFEDIT_FEAT_DQT];
        json_object_object_get_ex(jframe, "data", &jdata);
    }

    while (len >= 65) {
        PutBitContext *saved;
        json_object *ffe_dqt_cur;
        ...
        if ( (s->avctx->ffedit_export & (1 << FFEDIT_FEAT_DQT)) != 0 )
        {
            ffe_dqt_cur = json_object_new_array();
            json_object_array_put_idx(jdata, index, ffe_dqt_cur);
        }
        else if ( (s->avctx->ffedit_import & (1 << FFEDIT_FEAT_DQT)) != 0 )
        {
            ffe_dqt_cur = json_object_array_get_idx(jdata, index);
        }
        ...
        for (i = 0; i < 64; i++) {
            ...
            if ( (s->avctx->ffedit_import & (1 << FFEDIT_FEAT_DQT)) != 0 )
            {
                json_object *jval = json_object_array_get_idx(ffe_dqt_cur, i);
            ...
            if ( (s->avctx->ffedit_export & (1 << FFEDIT_FEAT_DQT)) != 0 )
            {
                ...
                json_object_array_put_idx(ffe_dqt_cur, i, jval);
            }
...
int ff_mjpeg_decode_dht(MJpegDecodeContext *s)
{
    ...
    {
        AVFrame *f = s->picture_ptr;
        json_object *jframe = f->ffedit_sd[FFEDIT_FEAT_DHT];
    ...
    while (len > 0) {
        ...
        if ( (s->avctx->ffedit_export & (1 << FFEDIT_FEAT_DHT)) != 0 )
        {
            ...
            for ( i = 0; i < 16; i++ )
            {
                ...
                json_object_set_serializer(jvals, ffe_int_line_to_json_string,
                                           (void *) -1, NULL);
        ...
        int i)
{
    ...
    AVFrame *f = s->picture_ptr;
    json_object *jframe = f->ffedit_sd[feature];
        ...
        int block)
{
    ...
    if ( (s->avctx->ffedit_import & (1 << FFEDIT_FEAT_Q_DC)) != 0 )
...
ffe_dct_scan_new(MJpegDecodeContext *s)
{
    ...
    if ( (s->avctx->ffedit_export & (1 << FFEDIT_FEAT_Q_DC)) != 0 )
    {
        line_func = ffe_int_line_to_json_string;
        ud = (void *) 5;
    }
                    ...
                    if (!s->progressive) {
                        ...
                        if (Ah)
                        {
                            ...
                            if ( (s->avctx->ffedit_import & (1 << FFEDIT_FEAT_Q_DCT)) != 0
                              || (s->avctx->ffedit_import & (1 << FFEDIT_FEAT_Q_DC)) != 0 )
                            {
                                code = (ffe_dct_get(s, i, mb_y, mb_x, j, 0) == 0) ? 0 : 1;
                            }
                            if ( (s->avctx->ffedit_apply & (1 << FFEDIT_FEAT_Q_DCT)) != 0
                              || (s->avctx->ffedit_apply & (1 << FFEDIT_FEAT_Q_DC)) != 0 )
                            {
                            ...
                            if ( (s->avctx->ffedit_export & (1 << FFEDIT_FEAT_Q_DCT)) != 0
                              || (s->avctx->ffedit_export & (1 << FFEDIT_FEAT_Q_DC)) != 0 )
                            {
                                ffe_dct_set(s, i, mb_y, mb_x, j, 0, code);
                            }
                          ...
                          AVPacket *avpkt)
{
    ...
    {
        ret = ffe_transplicate_init(avctx, &s->ffe_xp, 0x100000);
        if ( ret < 0 )
            return ret;
    }
...
AVCodec ff_mjpeg_decoder = {
    ...
    .flush          = decode_flush,
    .capabilities   = AV_CODEC_CAP_DR1 | AV_CODEC_CAP_FFEDIT_BITSTREAM,
    .max_lowres     = 3,
