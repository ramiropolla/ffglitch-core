...
{
    AVFrame *f = s->current_picture_ptr->f;
    json_object *jframe = f->ffedit_sd[FFEDIT_FEAT_QSCALE];
...
ffe_mpeg12_export_init(MpegEncContext *s)
{
    ...
    if ( (s->avctx->ffedit_export & (1 << FFEDIT_FEAT_QSCALE)) != 0 )
    {
        ...
        ctx->mb = ffe_jblock_new(s->mb_width, s->mb_height,
                                  ffe_int_line_to_json_string,
                                  (void *) 4);
    ...
    if ( (s->avctx->ffedit_export & (1 << FFEDIT_FEAT_Q_DCT)) != 0 )
    {
        ...
        jframe = ffe_jmb_new(s->mb_width, s->mb_height,
                              nb_components,
                              v_count, h_count,
                              NULL, NULL);
                         ...
                         int *got_output, const uint8_t *buf, int buf_size)
{
    ...
    for (;;) {
        ...
        opb = ffe_transplicate_pb(&s2->ffe_xp);
                             ...
                             int *got_output, AVPacket *avpkt)
{
    ...
    {
        ret = ffe_transplicate_init(avctx, &s2->ffe_xp, 0x100000);
        if ( ret < 0 )
            return ret;
    }
