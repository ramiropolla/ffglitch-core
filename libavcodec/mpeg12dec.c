                                           ...
                                           int16_t *block, int16_t *qblock, int n)
{
...

    {
        ...
        for (;;) {
            get_rl_vlc2(&level, &run, &s->gb, ff_mpeg1_rl_vlc,
                        TEX_VLC_BITS, 2, 0);
...
//    motion_type = 0; /* avoid warning */
    if (IS_INTRA(mb_type)) {
        ...
        if (s->concealment_motion_vectors) {
            ...
            ffe_mpeg12_mv_select(&mbctx, s, 0, 0);

            ffe_mpeg12_mv_delta_init_mb(&mbctx_delta, s, 1, 1);
            ffe_mpeg12_mv_delta_select(&mbctx_delta, s, 0, 0);

            s->mv[0][0][0]      =
            s->last_mv[0][0][0] =
            s->last_mv[0][1][0] = ffe_decode_mpegmv(&mbctx, &mbctx_delta,
                                                    s, s->mpeg_f_code[0][0],
                                                    s->last_mv[0][0][0], 0);
            s->mv[0][0][1]      =
            s->last_mv[0][0][1] =
            s->last_mv[0][1][1] = ffe_decode_mpegmv(&mbctx, &mbctx_delta,
                                                    s, s->mpeg_f_code[0][1],
                                                    s->last_mv[0][0][1], 1);
        ...
        if (mb_type & MB_TYPE_ZERO_MV) {
            ...
            case MT_FRAME: /* or MT_16X8 */
                if (s->picture_structure == PICT_FRAME) {
                    ...
                    ffe_mpeg12_mv_delta_init_mb(&mbctx_delta, s, 2, 1);
                    for (i = 0; i < 2; i++) {
                        if (USES_LIST(mb_type, i)) {
                            ffe_mpeg12_mv_select(&mbctx, s, i, 0);
                            ffe_mpeg12_mv_delta_select(&mbctx_delta, s, i, 0);

                            /* MT_FRAME */
                            s->mv[i][0][0]      =
                            s->last_mv[i][0][0] =
                            s->last_mv[i][1][0] =
                                ffe_decode_mpegmv(&mbctx, &mbctx_delta,
                                                  s, s->mpeg_f_code[i][0],
                                                  s->last_mv[i][0][0], 0);
                            s->mv[i][0][1]      =
                            s->last_mv[i][0][1] =
                    ...
                    ffe_mpeg12_mv_delta_init_mb(&mbctx_delta, s, 2, 2);
                    for (i = 0; i < 2; i++) {
                        if (USES_LIST(mb_type, i)) {
                            /* MT_16X8 */
                            for (j = 0; j < 2; j++) {
                                s->field_select[i][j] = get_bits1(&s->gb);
                                ffe_mpeg12_mv_select(&mbctx, s, i, j);
                                ffe_mpeg12_mv_delta_select(&mbctx_delta, s, i, j);
                                for (k = 0; k < 2; k++) {
                                    val = ffe_decode_mpegmv(&mbctx, &mbctx_delta,
                                                            s, s->mpeg_f_code[i][k],
                                                            s->last_mv[i][j][k], k);
                                    s->last_mv[i][j][k] = val;
                                    s->mv[i][j][k]      = val;
            ...
            case MT_DMV:
                ...
                for (i = 0; i < 2; i++) {
                    if (USES_LIST(mb_type, i)) {
                        ...
                        mx = ffe_decode_mpegmv(&mbctx, &mbctx_delta,
                                               s, s->mpeg_f_code[i][0],
                                               s->last_mv[i][0][0], 0);
                        s->last_mv[i][0][0] = mx;
                        s->last_mv[i][1][0] = mx;
                        dmx = get_dmv(s);
                        my  = ffe_decode_mpegmv(&mbctx, &mbctx_delta,
                                                s, s->mpeg_f_code[i][1],
...
static int ffe_transplicate_bits_init_get_bits(
        ...
        int bit_size)
{
    int ret = init_get_bits(gb, buf, bit_size);
    gb->pb = xp->pb;
    return ret;
}
                                ...
                                int buf_size)
{
    ...
    ret = ffe_mpeg12_init_get_bits8(&s->ffe_xp, &s->gb, buf, buf_size);
    if (ret < 0)
        return ret;
    ...
    for (;;) {
        ...
        if (s->mb_skip_run == -1) {
            ...
            for (;;) {
                ...
                code = get_vlc2(&s->gb, ff_mbincr_vlc,
                                MBINCR_VLC_BITS, 2);
                if (code < 0) {
                                 ...
                                 const uint8_t *buf, int buf_size)
{
    ...
    int ret = ffe_mpeg12_init_get_bits8(&s->ffe_xp, &s->gb, buf, buf_size);
    if (ret < 0)
        return ret;
                         ...
                         int *got_output, const uint8_t *buf, int buf_size)
{
    ...
    for (;;) {
        ...
        case PICTURE_START_CODE:
            ...
            break;
        case EXT_START_CODE:
            ret = ffe_mpeg12_init_get_bits8(&s2->ffe_xp, &s2->gb, buf_ptr, input_size);
            if (ret < 0)
                return ret;
